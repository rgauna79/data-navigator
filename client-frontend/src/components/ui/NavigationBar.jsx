...
import {
  faBars, faXmark, faSignOutAlt,
  faHome, faFileExcel, faDatabase, faClockRotateLeft, faCodeCompare,
              ...
              {visibleLinks.map(({ to, label, icon }) => (
                <Link key={to} to={to}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    isActive(to) ? "bg-gray-700 text-white" : "text-gray-400 hover:text-white hover:bg-gray-800"
                  }`}
                >
          ...
          <div className="hidden md:flex items-center gap-3">
            {isLoggedIn ? (
              ...
              <>
                ...
                <Link to="/register" className="text-sm bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg transition-colors font-medium">
                  Sign up
                </Link>
              </>
            )}
        ...
        <div className="md:hidden border-t border-gray-700/60 bg-gray-900 px-4 py-3 space-y-1">
          ...
          <div className="border-t border-gray-700/60 pt-3 mt-3">
            {isLoggedIn ? (
              ...
              <div className="space-y-1">
                <Link to="/login" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-400 hover:text-white hover:bg-gray-800 transition-colors">
                  Sign in
                </Link>
                <Link to="/register" className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors">
                  Sign up
                </Link>
              </div>
            )}
